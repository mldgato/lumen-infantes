...
class EnrollmentList extends Component
{
    ...
    public function updatedSearchStudent(): void
    {
        ...
        $this->searchResults = User::whereHas('student')
            ->where(
                fn ($q) => $q->where('name', 'like', '%'.$this->searchStudent.'%')
                    ->orWhere('cui', 'like', '%'.$this->searchStudent.'%')
                    ->orWhereHas(
                        'student',
                        fn ($q) => $q->where('carne', 'like', '%'.$this->searchStudent.'%')
    ...
    {
        if (! EnrollmentPeriod::activeForEnrollments()) {
            $this->dispatch('toastMessage', ['type' => 'error', 'message' => 'No hay un período de inscripciones activo.']);
    ...
    public function render()
    {
        ...
        $enrollmentPeriodActive = EnrollmentPeriod::activeForEnrollments();
